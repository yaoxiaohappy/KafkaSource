...
public final class RecordAccumulator {
    ...
    public RecordAccumulator(int batchSize,
        ...
        //xiaoyao  注意这个copy on write 数据结构
        //这个数据结构是kafka自己设计的，这个结构在jdk里没有。
        //这个数据结构设计得非常好，因为这个这个结构整体提升了批次封装的性能
        //jdk中juc包里有CopyOnWriteArraylist
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            synchronized (dq) {
                ...
                //步骤二：尝试往队列里添加数据
                //一开始肯定失败，因为目前只有队列，数据是需要存储到批次对象里的，目前还没分配内存
                RecordBatch last = dq.peekLast();
                //第一次来这里是没有批次的，所以last为null
                if (last != null) {
            ...
            /*
            步骤三：
            计算一个批次的大小在消息的大小和批次的大小中取一个最大值，
            有可能一个消息的大小比批次的大小还大
            默认一个批次的代码是16k
            如果我们生产者发送数据的时候，所有消息都大于16k，那么一条消息就是一个批次，消息是一条一条被发送的
            那批次概念的设计就没意义了。
            ...
            //注意这里的分段加锁，该加锁的地方加锁，不该加的地方不要加。
            //在高并发调用的地方，必须要保证线程安全，同时为了提升性能，选择性地去加锁，
            //假设有三个线程进来往同一个队列里写入数据，三个线程就都能申请到16k的内存，可以同时写入多个数据，如果重复利用了内存后
            //下面调用deallocate释放掉自己的内存
            synchronized (dq) {
                ...
                /*
                步骤五：再次尝试将数据写入到批次里。
                但是代码执行到这还是会失败，目前虽然已经分配了内存，但是还没有创建批次
                 */
                RecordBatch last = dq.peekLast();
                if (last != null) {
                    FutureRecordMetadata future = last.tryAppend(timestamp, key, value, callback, time.milliseconds());
                ...
                /*
                步骤六：根据内存大小封装批次，new一个record batch，第三次尝试往批次中写入数据，这时候代码执行成功
                 */
            ...
            if (!muted.contains(tp)) {
                synchronized (dq) {
                    ...
                    while (batchIterator.hasNext()) {
                        ...
                        //xiaoyao：这里检查是否超时，同时，如果超时里，就抛出超时的exception
        ...
        for (Map.Entry<TopicPartition, Deque<RecordBatch>> entry : this.batches.entrySet()) {
            ...
            if (leader == null) {
                ...
                synchronized (deque) {
                    ...
                    timeToWaitMS：最多等待多久
                    timeLeftMS：还要再等多久
                    full：是否有写满的批次，批次写满了，就得发送
                    expire：时间到了批次没写满，也得发送
                    close：关闭线程
                    flushInProgres：生产这执行flush操作
                    ...
                    if (batch != null) {
                        ...
                        //各种条件达到了就加上，如果需要重试了，就不添加leader了
