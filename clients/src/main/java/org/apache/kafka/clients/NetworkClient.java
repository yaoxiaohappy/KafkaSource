...
public class NetworkClient implements KafkaClient {
    ...
    public boolean isReady(Node node, long now) {
        ...
        // priority
        //canSendRequest 是否可以发送消息
        //isUpdateDue 更新元数据的时候不能发送消息
    ...
    private boolean canSendRequest(String node) {
        /**
         * 生产者维护了多个连接缓存着，跟我们的docker的节点数是一样的
         * 判断缓存里是否已经把连接建立好了
         * javaNIO selector
         * 绑定了多个kafkaChannel，每个kafakChannel代表一个连接
         * 一个往多个主机发送，最多能容忍五个消息发送出去，但还没接受响应
         * maxInFlightRequestsPerConnection=5 会影响发送数据的顺序，如果要严格保证发送顺序的话，maxInFlightRequestsPerConnection需要设置为1
         *
         */
    ...
    private void doSend(ClientRequest request, long now) {
        ...
        //向组建里存请求（还没有响应的），默认最多能存五个请求
        //每个连接最终忍受五个连接收不到请求
        //如果请求发出去了也成功接受到响应，就会移除这个请求
    ...
    public List<ClientResponse> poll(long timeout, long now) {
        ...
        //就是在这里处理响应的networkreceive，处理的数据结构是completed

        handleCompletedSends(responses, updatedNow);
        //步骤三：处理响应的步骤，响应里就由要处理的元数据
        /*
        其实kafka获取元数据的流程和我们发送的消息是一摸一样的
        获取元数据——》判断网络是否建立好——》建立网络连接——》发送请求（获取元数据的请求）
        ——》服务端发回来响应
         */
    ...
    private void handleCompletedReceives(List<ClientResponse> responses, long now) {
        for (NetworkReceive receive : this.selector.completedReceives()) {
            ...
            /**
             * kafka允许发送请求，每个连接容忍5个发送出去，但是还没有接受到响应的请求。
             * 这里从inFlightRequests 移除已经接受到响应的请求
             */
            ...
            if (!metadataUpdater.maybeHandleCompletedReceive(req, now, body))
                //解析完成后封装成ClientResponse，这个就是响应的内容
                //body是响应的内容
                //req是响应的信息
    ...
    class DefaultMetadataUpdater implements MetadataUpdater {
        ...
        private void handleResponse(RequestHeader header, Struct body, long now) {
            this.metadataFetchInProgress = false;
            //解析从服务端发回的二进制解析对象，响应中就获取到拉取元数据信息
        ...
        private void maybeUpdate(long now, Node node) {
            ...
            if (canSendRequest(nodeConnectionId)) {
                ...
                if (metadata.needMetadataForAllTopics())
                    ...
                    //一般默认走这一个方法，拉去发送消息对应的topic请求
                    metadataRequest = new MetadataRequest(new ArrayList<>(metadata.topics()));
                // 这里metadataRequest 就是我们封装好的发送请求
